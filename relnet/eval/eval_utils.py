...
def get_values_for_g_list(agent, g_list, initial_obj_values, validation, make_action_kwargs):
    ...
    # print(f"total edge budget agent is given is {agent.environment.edge_budgets[0]}")
    # print(f"this means {2 * agent.environment.edge_budgets[0]} timesteps.")
    ...
    while not agent.environment.is_terminal():
        ...
        action_kwargs['training'] = False
        agent.obj_fun_eval_count = 0
        agent.log_timings_if_required(t, "before", len(g_list), agent.obj_fun_eval_count)
        list_at = agent.make_actions(t, **action_kwargs)
        agent.log_timings_if_required(t, "after", len(g_list), agent.obj_fun_eval_count)
